...
mlbids <- read.csv("data/MLB_player_IDs.csv")
...
View(plays %>% filter(hit_location == 1, events == "force_out"))
...
sp.outs <- left_join(sp.outs, mlbids, by = c("pitcher" = "MLBID"))
sp.outs$MLBNAME[which(sp.outs$pitcher == 666201)] <- "Alek Manoah"
sp.outs$MLBNAME[which(sp.outs$pitcher == 669145)] <- "Bruce Zimmermann"
